import {createCell, ITcdCell, TypeCell} from './TcdCell';
...
export const generateMock = (rowCount: number) => {
    ...
    // Ligne 0 : Headers
    columns.forEach((col: ITcdColumn, x: number) => {
        cells.push(
            createCell(TypeCell.none, {x, y: 0, xSpan:1, ySpan:1}, col.name, col, true)
        );
    });
    ...
    for (let y = 1; y <= rowCount; y++) {
        columns.forEach((col, x) => {
            ...
            cells.push(
                createCell(TypeCell.none, {x, y, xSpan:1, ySpan:1}, value, col, false)
            );
        });
    }

    const viewColumns: TcdViewColumn[] = columns.map((c:ITcdColumn)=>{
        return {width: c.width}
    })

    return {cells, viewColumns};
}
